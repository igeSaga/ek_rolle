...
rolle_build_osm$areaOrig<-osm.inf$areaOSM
...
diff.sp$areaCrop2<-rgeos::gArea(diff.sp,byid=TRUE)

diff.df<-merge(diff.sp@data,osm.inf,by="idGeom")
diff.df$areaDiff<-diff.df$areaCrop/diff.df$areaOSM

rolle_build_osm_excl<-diff.sp[diff.sp$idGeom%in%diff.df$idGeom[diff.df$areaDiff>0.9],]
